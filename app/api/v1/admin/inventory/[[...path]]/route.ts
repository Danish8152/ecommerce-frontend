...
const proxyAdminInventoryRequest = async (request: NextRequest, context: RouteContext) => {
  ...
  const pathnameSuffix = path && path.length > 0
    ...
    : "";
  const targetUrl = `${apiBaseUrl}/api/v1/admin/inventory${pathnameSuffix}${request.nextUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: "Unable to reach admin inventory service",
...
export async function GET(request: NextRequest, context: RouteContext) {
  return proxyAdminInventoryRequest(request, context);
}
