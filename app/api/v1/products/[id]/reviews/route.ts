...
const proxyProductReviewsRequest = async (
  request: NextRequest,
  ...
  const targetUrl = `${apiBaseUrl}/api/v1/products/${encodeURIComponent(
    id
  )}/reviews${request.nextUrl.search}`;
  ...
  try {
    ...
    return NextResponse.json(
      {
        message: "Unable to reach product reviews service",
...
export async function GET(request: NextRequest, context: RouteContext) {
  return proxyProductReviewsRequest(request, context);
}
