...
import { ApiProduct } from "@/lib/productMapping";
...
const normalizeImageSrc = (rawSrc: string) => {
  ...
  if (src.startsWith("uploads/")) {
    if (UPLOADS_BASE_URL) return `${UPLOADS_BASE_URL}/${src}`;
    return `/${src}`;
  }
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    if (!productId) {
      setLoadError("No product ID specified");
    ...
    const loadProduct = async () => {
      ...
      try {
        ...
        const product = response.data?.data as ApiProduct | undefined;

        if (!isMounted || !product) {
          if (isMounted) setLoadError("Product not found");
          return;
        }

        // Extract thumbnail
        const thumb = normalizeImageSrc(String(product.thumbnail || ""));
        setExistingThumbnail(thumb);
        setImagePreview(thumb);

        // Extract gallery from media (skip first if it's the thumbnail)
        const mediaUrls = (product.media || [])
          .map((m) => normalizeImageSrc(String(m.url || "")))
          .filter(Boolean);
        const galleryUrls = mediaUrls.filter((url) => url !== thumb);
        setExistingGallery(galleryUrls);

        // Extract variant data
        const apiVariants = product.variants || [];
        const hasVariants = Boolean(product.hasVariants) && apiVariants.length > 0;

        // Build attribute list from variants
        const attributeMap = new Map<string, Set<string>>();
        for (const v of apiVariants) {
          for (const av of v.attributeValues || []) {
            const attrName = String(av.attribute?.name || "").trim();
            const attrValue = String(av.attributeValue?.value || "").trim();
        ...
        const attributes: ProductAttribute[] = Array.from(attributeMap.entries()).map(
          ([name, values]) => ({
            name,
        ...
        // Build variant rows
        const variantRows: ProductVariant[] = apiVariants.map((v) => {
          const attrs = (v.attributeValues || []).map((av) => ({
            name: String(av.attribute?.name || ""),
            value: String(av.attributeValue?.value || ""),
          }));

          const id = attrs
            .map((a) => `${makeAttributeCode(a.name)}:${slugify(a.value)}`)
            .join("|");

          const variantPrice = v.price !== undefined && v.price !== null ? String(v.price) : "";
          const variantComparePrice = v.comparePrice !== undefined && v.comparePrice !== null ? String(v.comparePrice) : "";
          const variantStock = v.inventory?.quantity !== undefined ? String(v.inventory.quantity) : (v.inventory?.availableQuantity !== undefined ? String(v.inventory.availableQuantity) : "0");

          return {
            id,
            combinationLabel: attrs.map((a) => `${a.name}: ${a.value}`).join(" / "),
            selected: true,
            sku: String(v.sku || ""),
            price: variantPrice,
            ...
            stock: variantStock,
            barcode: "",
            image: normalizeImageSrc(String(v.image || "")),
        ...
        // Extract meta entries for SEO
        const metaEntries = product.metaEntries || [];
        const seoTitleEntry = metaEntries.find((e) => e.metaKey === "seoTitle");
        const seoDescEntry = metaEntries.find((e) => e.metaKey === "seoDescription");
        ...
        setFormData({
          ...
          sku: simpleSku,
          price: simplePrice,
          comparePrice: simpleComparePrice,
          stock: simpleStock,
          ...
          hasVariants,
          seoTitle: String(seoTitleEntry?.metaValue || ""),
          seoDescription: String(seoDescEntry?.metaValue || ""),
          attributes: attributes.length > 0 ? attributes : [{ name: "", valuesText: "" }],
          variants: variantRows,
        });
      } catch (error: unknown) {
        if (isMounted) {
          setLoadError(getErrorMessage(error, "Failed to load product"));
        }
      } finally {
  ...
  const handleRemoveExistingGallery = (index: number) => {
    setExistingGallery((previous) => previous.filter((_, currentIndex) => currentIndex !== index));
  };

  const handleAttributeChange = (index: number, key: keyof ProductAttribute, value: string) => {
    setFormData((previous) => {
      const nextAttributes = [...previous.attributes];
      nextAttributes[index] = { ...nextAttributes[index], [key]: value };
      return { ...previous, attributes: nextAttributes };
    });
  };
  ...
  const handleVariantFieldChange = (index: number, key: keyof ProductVariant, value: string | boolean) => {
    setFormData((previous) => {
      ...
      nextVariants[index] = { ...nextVariants[index], [key]: value } as ProductVariant;
  ...
  const filteredVariants = useMemo(() => {
    ...
    if (!normalizedSearch) return formData.variants;
  ...
  const submitUpdate = async (intent: "draft" | "publish") => {
    setIsSaving(true);
    ...
    try {
      ...
      if (!trimmedTitle) throw new Error("Product name is required");
      if (!trimmedSlug) throw new Error("Slug is required");
      if (!trimmedDescription) throw new Error("Product description is required");

      // Handle thumbnail: upload new or keep existing
      ...
      if (selectedImageFile) {
        ...
        const uploadedPath = thumbnailUploadResponse.data?.data?.path as string | undefined;
        if (!uploadedPath) throw new Error("Thumbnail upload failed");
      ...
      const newGalleryPaths = await Promise.all(
        galleryFiles.map(async (file, index) => {
          ...
          if (!path) throw new Error(`Gallery upload failed for ${file.name}`);
          return path;
        }),
      );
      ...
      // Build media array from existing + new gallery
      const allGalleryPaths = [...existingGallery, ...newGalleryPaths];
      payload.media = [
        {
          url: thumbnailPath,
          mediaType: "image",
      ...
      if (!formData.hasVariants) {
        ...
        if (!Number.isInteger(stock) || stock < 0) throw new Error("Stock must be a valid non-negative integer");
      ...
      toast.success(intent === "publish" ? "Product updated successfully!" : "Draft saved successfully!");
  ...
  if (isLoadingProduct) {
    return (
      <div className="mx-auto max-w-screen-2xl px-4 pb-16 sm:px-6 lg:px-8">
        <div className="flex items-center justify-center gap-3 py-32 text-gray-500">
          <Loader2 size={24} className="animate-spin" />
          <span className="text-sm font-bold uppercase tracking-widest">Loading product...</span>
        </div>
      </div>
    );
  }
  ...
  if (loadError) {
    return (
      <div className="mx-auto max-w-screen-2xl px-4 pb-16 sm:px-6 lg:px-8">
        <Link href="/admin/products" className="inline-flex items-center gap-2 text-sm font-bold text-gray-500 transition-colors hover:text-black">
          <ArrowLeft size={16} /> Back to Products
        </Link>
        <div className="mt-8 rounded-3xl border border-red-100 bg-red-50 p-12 text-center">
  ...
  return (
    <div className="mx-auto max-w-screen-2xl px-4 pb-16 sm:px-6 lg:px-8">
      ...
      <div className="mt-4 mb-8 max-w-3xl">
        <h2 className="text-3xl font-black uppercase tracking-tighter text-gray-900 sm:text-4xl">Edit Product</h2>
        <p className="mt-2 text-sm font-medium text-gray-500 sm:text-base">
          Update the product details, pricing, images, and variants.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1.72fr)_minmax(280px,0.42fr)]">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <div className="md:col-span-2 rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <label className="inline-flex items-center gap-3">
                      <span className={`text-xs font-black uppercase tracking-wider ${!formData.hasVariants ? "text-black" : "text-gray-400"}`}>
                        Simple
                      </span>
                      <button
                        type="button"
                ...
                {!formData.hasVariants ? (
                  <>
                    ...
                    <div>
                      <label className="mb-2 block text-xs font-black uppercase text-gray-700">Stock</label>
                      <input
                        type="number"
                        min={0}
                        required
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                {existingGallery.length > 0 ? (
                  <div className="space-y-2">
                    <label className="block text-xs font-black uppercase text-gray-700">Current Gallery Images</label>
                    {existingGallery.map((url, index) => (
                      <div key={`existing-${index}`} className="flex items-center gap-2 rounded-xl bg-gray-50 px-3 py-2">
                        <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-lg bg-gray-200">
                          <Image src={url} alt={`Gallery ${index + 1}`} fill sizes="40px" className="object-cover" unoptimized />
                        </div>
                        <span className="flex-1 truncate text-[11px] font-semibold text-gray-600">{url.split("/").pop()}</span>
                        <button type="button" onClick={() => handleRemoveExistingGallery(index)} className="text-gray-400 hover:text-red-600">
                          <Trash2 size={14} />
                        </button>
                      </div>
                    ))}
                ...
                <div className="space-y-2">
                  <label className="block text-xs font-black uppercase text-gray-700">Add More Gallery Images</label>
                  <input
                    type="file"
            ...
            <section className="rounded-3xl border border-gray-100 bg-white p-6 shadow-xl sm:p-8">
              <h3 className="mb-4 flex items-center gap-2 border-b border-gray-100 pb-4 text-lg font-black uppercase tracking-tight text-gray-900">
                <Tag size={20} className="text-[#facc15]" /> Update Controls
              </h3>
              ...
              <div className="space-y-3">
                ...
                >
                  ...
                  {isSaving && pendingIntent === "draft" ? "Saving Draft..." : "Save as Draft"}
                </button>
                ...
                >
                  ...
                  {isUploading ? "Uploading Files..." : isSaving && pendingIntent === "publish" ? "Updating..." : "Update Product"}
                </button>
